...
namespace BepuPhysics
{
    ...
    public interface IRayHitHandler
    {
        ...
        /// </summary>
        /// <param name="ray">Information about the ray associated with this hit.</param>
        /// <param name="maxDistance">Maximum distance along the ray that the traversal is allowed to go in units of ray direction length. Can be set to limit future tests.</param>
        /// <param name="distance">Distance along the ray to the impact in units of ray direction length. In other words, hitLocation = ray.Origin + ray.Direction * t.</param>
        /// <param name="normal">Surface normal at the hit location.</param>
        /// <param name="collidable">Collidable hit by the ray.</param>
        /// <param name="childIndex">Index of the hit child. For convex shapes or other types that don't have multiple children, this is always zero.</param>
        void OnRayHit(in RayData ray, ref float maxDistance, float distance, in Vector3 normal, CollidableReference collidable, int childIndex);
    }
